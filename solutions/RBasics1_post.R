...
data2<- read.table(file = "Datasets/Billboard_Hot_weekly_charts/Hot100AudioFeatures.tsv", header = TRUE) 
data2
...
###Slide 17--------------------------------------------------
#Expore data
head(my.my.data1) #check first few rows of data
...
class(my.data1) #check class of my.data1
ncol(my.data1) #check number of column
nrow(my.data1) #check number of row
colnames(my.data1) #list column names
class(my.data1$Age) #check data type of age
class(my.data1$Gender) #check data type of gender
...
###Slide 18--------------------------------------------------
##lets test package
install.packages("ggplot2") #install a library package
...
union(country_mydata1, country_mydata2)
#Which county is unique to country_mydata2
setdiff(country_mydata2, country_mydata1)
#Which county is unique to country_mydata1
setdiff(country_mydata1, country_mydata2)
...
my.data2 <- iris
head(iris)
class(my.data2$Sepal.Length)
class(my.data2$Species)
...
mean(my.data2$Petal.Length)
median(my.data2$Petal.Length)
sd(my.data2$Petal.Length)
...
country_mydata1 <- unique(my.data1$Country)
if("Finland" %in% country_mydata1){
  print("Finland is included")
} else{
  print("Finland is not included")
}

###Slide 23--------------------------------------------------
#Save data
save(my.my.data1, file = "/Users/harukamitsuhashi/Desktop/R_Workshop_2022/workshop_my.data1.Rds")
write.csv(my.my.data1, "/Users/harukamitsuhashi/Desktop/R_Workshop_2022/workshop_my.data1.csv")

#Exercise9 try saving your own variables and workspace (hint: use save.image)

#Example - save your command history
savehistory(file = "C:/Users/malosree/Desktop/R_Workshop_2022/Workshop_commands.Rhistory")
