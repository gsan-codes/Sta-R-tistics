...
r_orig <- cor.test(data$Sepal.Length, data$Petal.Length)$estimate

if(r_orig > 0) {
  2*(sum(permuted_r >= r_orig)/length(permuted_r))
} else {
  2*(sum(permuted_r <= r_orig)/length(permuted_r))
}

#Repeat with a different pair of variables and get the p-value demonstration, can run this a few times to see by how much the p_value changes
#https://www.uvm.edu/~statdhtx/StatPages/Randomization%20Tests/RandomCorr/randomization_Correlation.html

permuted_r= c()

for (i in 1:1000){
  
  permuted_data= data.frame( data$Sepal.Length[sample(nrow(data))], data$Sepal.Width[sample(nrow(data))])
  colnames(permuted_data) = c("Sepal.Length", "Sepal.Width")
  temp=cor.test(permuted_data$Sepal.Length, permuted_data$Sepal.Width)
  permuted_r[i]= temp$estimate
...
r_orig <- cor.test(data$Sepal.Length, data$Sepal.Width)$estimate

if(r_orig > 0) {
  2*(sum(permuted_r >= cor.test(data$Sepal.Length, data$Sepal.Width)$estimate))/length(permuted_r)
} else {
  2*(sum(permuted_r <= cor.test(data$Sepal.Length, data$Sepal.Width)$estimate))/length(permuted_r)
}
...
#Bootstrapping a p-value for a correlation coefficient
#https://www.uvm.edu/~statdhtx/StatPages/Randomization%20Tests/BootstCorr/bootstrapping_correlations.html

bootstrap_r = c()

for( i in 1:10000) {
  bootstrap_data= data.frame( data[sample(nrow(data), replace = TRUE), c("Sepal.Length", "Sepal.Width")])
  bootstrap_r[i] <- cor(bootstrap_data$Sepal.Length, bootstrap_data$Sepal.Width)
}

hist(bootstrap_r)
quantile(bootstrap_r, 0.025)
quantile(bootstrap_r, 0.975)

if(cor(data$Sepal.Length, data$Petal.Length) > 0) {
  2* sum(bootstrap_r < 0)/length(bootstrap_r)
} else {
  2* sum(bootstrap_r > 0)/length(bootstrap_r)
}
...
for( i in 1:10000) {
  bootstrap_data= data.frame( data[sample(nrow(data), replace = TRUE), c("Sepal.Length", "Petal.Length")])
  bootstrap_r[i] <- cor(bootstrap_data$Sepal.Length, bootstrap_data$Petal.Length)
}
...
protein = read.csv('C:/Users/Debajyoti Saha/Documents/GitHub/Sta-R-tistics/datasets/MouseProtein_Kaggle/Data_Cortex_Nuclear.csv')
...
#Maybe specifically introduce the p.adjust functions
