...
names(table(iris$Sepal.Length)[which.max(table(iris$Sepal.Length))])
...
smallest_5 <- function(x) {
  head(x[order(x)])
}

apply(iris, 2, smallest_5)
...
t(t(data[1:4]) - colMeans(data[1:4], na.rm = TRUE))/apply(data[1:4], 2, sd, na.rm = TRUE) #I think we might need transpose because subtracting a vector from a dataframe seems to not go row by row by default
...
CI = function(data, conf=0.95){
  ...
  CI= qnorm(conf_level) * sd(data)/sqrt(length(data)) #guessed you meant to say qnorm(conf_level) rather than qnorm(0.975) here
...
t.test(sample1[1:5],sample2[1:5]) # or you can use the sample function
sample(1:length(sample1), 5)
...
data=read.csv("~/Documents/GitHub/Sta-R-tistics/data/EEG_effect_sizes/sample_sizes.csv")
...
for (ds in cohend){
  ...
  for (i in data$nsub_after_exc){
    power=pwr::pwr.anova.test(k=num_group, n=i, sig.level = 0.05, f = ds)
    one_group <- rbind(one_group, data.frame(power = power$power, effect_size = ds, sample_size = i))
...
for (k in cohend) {
  for (j in power_iter){
    pro=mean(one_group$power[one_group$effect_size==k] > j)
    proportions_bigger_than=rbind(proportions_bigger_than, data.frame(porportion=pro, effect_size=k, power= j))
  }
} 

# need to make sure that the effect size is a factor before plotting
proportions_bigger_than$effect_size= as.factor(proportions_bigger_than$effect_size)
